...
from server.resources.mappings import HARDWARE_API_CHOICES


class ResourceApiHardware(Resource):
    ...
    def get(self):
        ...
        if session_exists(remote_addr, self.logger):
            response = get_synced_response(HARDWARE_API_CHOICES, self.logger)
